...
import {
  explorePropertiesFetch,
  exploreSectionFetch,
} from "../../redux/pagesSlice";
import { useSelector, useDispatch } from 'react-redux';

const Explore = () => {
    const expSection = useSelector((state) => state.page.expSection);
    const expProperties = useSelector((state) => state.page.expProperties);
    const dispatch = useDispatch()

    useEffect(() => {
        dispatch(exploreSectionFetch())

        dispatch(explorePropertiesFetch ())
    }, [])
    ...
    return (
        <div>
            ...
            <div className="ml-[25px] flex flex-col flex-wrap gap-y-4 max-w-[1560px] pt-4  ">
                ...
                <img className="rounded-[12px] w-[1450px] pl-[40px]" src="https://t4.ftcdn.net/jpg/02/18/18/55/360_F_218185587_P4zituDtWJOfClUKL6merI0BgLMIxoeC.jpg" alt="img" />
                <h1 className="absolute bottom-[650px] font-medium left-[400px] text-[28px] text-white">Explore Page</h1>
